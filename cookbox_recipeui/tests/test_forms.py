...
class RecipeFormTest(CookboxBaseTest):
    ...
    def test_form_save_check_values_outside_related(self):
        ...
        compare_keys = set(RecipeForm.Meta.fields).intersection(set(form_data.keys()))
        for key in compare_keys:
            self.assertEqual(getattr(recipe, key), form_data[key])


class TestSearchForm(CookboxBaseTest):
    def test_convert_unit_stays_same(self):
        for unit in ("sec.", "min.", "hrs.", "days", "weeks", "months", "years"):
