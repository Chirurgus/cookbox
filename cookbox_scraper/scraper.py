...
def recipe_yield(dd):
    ...
    if isinstance(dd["recipeYield"], list):
        ...
        if len(dd["recipeYield"]) == 2:
            return Recipe_yield(dd["recipeYield"][1], dd["recipeYield"][0], None)
...
def scrape_instructions(recipe, ri):
    ...
    if all(isinstance(elem, str) for elem in ri):
    ...
    def _scrape_how_to_step(step, recipe, pos_counter):
        ...
        for norm_ins in normalize_instructions([step["text"]]):
            ins = recipe.instructions.create(instruction=norm_ins, position=pos_counter)
            pos_counter += 1
        ...
        if "image" in step.keys():
            ...
            save_image_url_in_field(
                ...
                "{id1}_{id2}_{id3}_thumb.png".format(
                    id1=str(recipe.id), id2=str(ins.id), id3=str(note.id)
                ),
            )
        return pos_counter

    if all(isinstance(elem, dict) for elem in ri):
        pos_counter = 0
        for item in ri:
            ...
            if item["@type"] == "HowToSection":
                if "name" in item.keys():
                    recipe.instructions.create(
                        instruction=item["name"], position=pos_counter
                    )
                    pos_counter += 1
...
def scrape_images(recipe, dd):
    ...
    if all([isinstance(image, dict) for image in dd["image"]]):
        dd["image"] = [image["url"] for image in dd["image"] if "url" in image.keys()]
    ...
    if "recipeIngredient" in data.keys():
        ...
        for idx, ing in enumerate(parse_ingredients(data["recipeIngredient"])):
            group.ingredients.create(
                quantity=ing[0], unit=ing[1], description=ing[2], position=idx
            )
