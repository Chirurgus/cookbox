...
class GlossaryListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["orphaned_articles"] = GlossaryArticle.objects.filter(
    ...
    def get_object(self):
        ...
        if entry.article is not None:
            ...
            entry.formatted_markdown = mark_safe(
                insert_links(markdownify(entry.article.body), entry.term)
            )
        return entry
...
class GlossaryEntryCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse("glossary-entry-detail", kwargs={"pk": self.object.id})
